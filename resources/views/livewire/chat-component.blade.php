<div x-data="data()" class="bg-gray-50 rounded-lg shadow-md border-gray-200 border overflow-hidden">
    <div class="grid grid-cols-3 divide-x divide-gray-200">

        <div class="col-span-1">
            <div class="bg-gray-100 h-16 w-16 flex justify-center items-center px-4">
            ...
            <div class="h-14 flex items-center bg-white px-4">
                <x-input wire:model="search" class="w-full" placeholder="Buscar chat" type="text" />
            </div>
            <div class="h-[calc(100vh-10.5rem)] overflow-auto border-t border-gray-200 ">

                @if ($this->chats->count() === 0 || $search)
                    <div class="px-4 py-3">
                        <h2 class="text-teal-600 text-lg font-semibold mb-4">Contactos</h2>
                        <ul class="space-y-4">
                            @forelse ($this->contacts as $contact)
                                <li class="cursor-pointer" wire:click="openChatContact({{ $contact }})">
                                    <div class="flex">
                                        ...
                                        <div class="flex-1 ml-5 border-b border-gray-200">
                                            <p class="text-gray-800 font-semibold ">{{ $contact->name }}</p>
                ...
                @else
                    @foreach ($this->chats as $chatItem)
                        <div wire:key="{{ $chatItem->id }}" wire:click="openChat({{ $chatItem }})"
                            ...
                            <div class="ml-4 w-[calc(100%-4rem)] py-4 border-b border-gray-200">
                                <div class="flex justify-between items-center">
                                    ...
                                    <div class="text-right">
                                        <p class="text-xs ">{{ $chatItem->lastMessageAt->format('h:i A') }}</p>
                                        {{-- <p class="text-xs">{{ $chatItem->messages->last()->created_at->format(' h:i A') }}</p> --}}
                                        @if ($chatItem->unreadMessage != 0)
                                            <span
                                                class="inline-flex items-center justify-center px-2 py-1.5 mr-2 text-xs font-bold leading-none text-white bg-green-600 rounded-full">
        ...
        <div class="col-span-2">
            @if ($contactChat || $chat)
                <div class="bg-gray-100 h-16 flex justify-start items-center px-3">
                    <figure class="">
                        @if ($chat)
                            <img class="h-10 w-10 object-cover object-center rounded-full" src="{{ $chat->image }}"
                                alt="">
                        @else
                            <img class="h-10 w-10 object-cover object-center rounded-full"
                    ...
                    <div class="ml-4">
                        ...
                        <p class="text-blue-500 text-xs " x-show="chatId == typingChatId">
                            Escribiendo...
                        </p>
                        @if ($this->active)
                            <p class="text-green-500 text-xs " x-show="chatId !== typingChatId" id="online">
                                Online
                            </p>
                        @else
                            <p class="text-gray-500 text-xs " x-show="chatId !== typingChatId" id="offline">
                                Offline
                            </p>
                        @endif
                ...
                <div class="h-[calc(100vh-11rem)] px-3 py-2 overflow-auto">
                    @foreach ($this->messages as $message)
                        <div class="flex {{ $message->user_id == auth()->id() ? 'justify-end' : '' }} mb-2">
                            <div
                                class="rounded px-3 py-2 {{ $message->user_id == auth()->id() ? 'bg-green-100' : 'bg-blue-100' }} ">
                                <p class="text-sm ">
                                    {{ $message->body }}
                                </p>
                                <p
                                    class="text-xs flex items-center text-gray-600 mt-1 {{ $message->user_id == auth()->id() ? 'justify-end ' : '' }}">
                                    {{ $message->created_at->format('h: i A') }}
                                    @if ($message->user_id == auth()->id())
                                        @if ($message->is_read)
                                            <i class="ml-2 ">
                                                <svg fill="#53BDEB" xmlns="http://www.w3.org/2000/svg" height="1em"
                                                    viewBox="0 0 512 512">
                                                    <path
                                        ...
                                        @else
                                            <i class="ml-2 ">
                                                <svg fill="#8696A0" xmlns="http://www.w3.org/2000/svg" height="1em"
                                                    viewBox="0 0 512 512">
                                                    <path
                                                        d="M505 174.8l-39.6-39.6c-9.4-9.4-24.6-9.4-33.9 0L192 374.7 80.6 263.2c-9.4-9.4-24.6-9.4-33.9 0L7 302.9c-9.4 9.4-9.4 24.6 0 34L175 505c9.4 9.4 24.6 9.4 33.9 0l296-296.2c9.4-9.5 9.4-24.7.1-34zm-324.3 106c6.2 6.3 16.4 6.3 22.6 0l208-208.2c6.2-6.3 6.2-16.4 0-22.6L366.1 4.7c-6.2-6.3-16.4-6.3-22.6 0L192 156.2l-55.4-55.5c-6.2-6.3-16.4-6.3-22.6 0L68.7 146c-6.2 6.3-6.2 16.4 0 22.6l112 112.2z" />
                                                </svg>
                ...
                <form class="bg-gray-100 h-16 flex justify-center items-center px-4" wire:submit.prevent="sendMessage">
                    <x-input wire:model="bodyMessage" type="text" class="flex-1" placeholder="Escribe un mensaje" />
                    <button class="flex-shrink-0 ml-4 text-gray-500">
            ...
            @else
                <div class="w-full h-full flex justify-center items-center">
                    <div>
                        <div class="_1135V">
                            <div>
                                <span data-testid="intro-md-beta-logo-dark" data-icon="intro-md-beta-logo-withe"
                        ...
                        Echo.private('App.Models.User.' + {{ auth()->id() }})
                            .notification((notification) => {
                                if (notification.type === 'App\\Notifications\\UserTyping') {
                                    this.typingChatId = notification.chatId
                                    setTimeout(() => {
